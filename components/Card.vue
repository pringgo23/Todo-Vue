<template>
    <div id="card-view" class="w-3/4">
        <div class="grid sm:grid-cols-12 md:grid-cols-3 md:gap-4">
            <div v-for="(tugas,index) in getView" :key="index">
                <div id="title" class="rounded-t-md h-10 mt-3 ">
                    <div class="flex justify-between p-2">
                        <h1 id="content-title" class="text-center text-white pl-9"
                            :style="{ 'text-decoration': (tugas.status ? 'line-through' : '') }">{{tugas.title}}</h1>
                        <div class="flex mr-2">
                            <button class="text-green-500 mr-2" @click="toggleStatus(i)"> V </button>
                            <button @click="showEdit(i)" class="mr-2 text-white"> <img class="w-4 bg-white"
                                    src="../assets/Image/edit.png" alt="edit" /> </button>
